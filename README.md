...
39 tools, all running 100% client-side:
...
| PDF to Markdown | Convert PDF to Markdown with heading detection and formatting |
...
Tests use [Vitest](https://vitest.dev/) and cover all 39 PDF services (236 tests). Each service has a co-located `.test.ts` file under `resources/js/Services/`. Canvas-dependent services (pdfjs-dist rendering) are tested via mocked canvas helpers.
